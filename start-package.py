...
if len(sys.argv) != 4:
    print("Usage: python3 start-package.py <url> <node-id> <pkg-dir>")
    sys.exit(1)

URL = sys.argv[1]
NODE = sys.argv[2]
PKG_DIR = os.path.abspath(sys.argv[3])
...
if not res.status == 200:
    print("Failed to send new package request")
    sys.exit(1)
...
if not resp.status == 200:
    print("Failed to send install package request")
    sys.exit(1)
